...
# from infrastructure.security.jwt_service import decode_refresh_token, create_access_token
...
#         result = register_user(request.name, request.email, request.password, request.role)
...
#         logger.error(f"Unexpected error during signup for {request.email}: {e}", exc_info=True)
...
#     except Exception as e:
#         logger.error(f"Unexpected error during login for {request.email}: {e}", exc_info=True)
#         raise HTTPException(status_code=500, detail="Internal Server Error")
