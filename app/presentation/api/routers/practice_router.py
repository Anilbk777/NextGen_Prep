...
from app.infrastructure.repositories.practice_session_repo import (
...
def start_session(
    ...
    try:
        ...
        return service.start_session(
            user_id=current_user.get("user_id"), topic_id=payload.topic_id
        )
    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            f"Unexpected error starting session for user {current_user.get("user_id")}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get(
    "/sessions/{session_id}/current-question", response_model=List[PracticeQuestionOut]
)
def get_current_question(
    ...
    try:
        ...
        logger.error(
            f"Unexpected error fetching current question for session {session_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail="Internal server error")


@router.post("/sessions/{session_id}/answer", response_model=PracticeAnswerOut)
def submit_answer(
    session_id: int,
    ...
    try:
        ...
        logger.error(
            f"Unexpected error submitting answer for session {session_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get("/sessions/{session_id}/summary", response_model=PracticeSessionSummaryOut)
def get_session_summary(
    ...
    try:
        ...
        logger.error(
            f"Unexpected error fetching summary for session {session_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail="Internal server error")
