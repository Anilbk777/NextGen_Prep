...
class PracticeSessionService:
    ...
    def start_session(self, user_id: int, topic_id: int) -> Dict[str, Any]:
        ...
        if len(easy) < 4 or len(medium) < 3 or len(hard) < 3:
            logger.warning(
                f"Not enough MCQs for topic {topic_id}: Easy({len(easy)}/4), Medium({len(medium)}/3), Hard({len(hard)}/3)"
            )
            raise HTTPException(
                status_code=400, detail="Not enough MCQs for this topic"
            )
        ...
        logger.info(
            f"Practice session {session.id} started successfully for user {user_id}"
        )
        return {"session_id": session.id, "total_questions": session.total_questions}
    ...
    def get_current_question(self, session_id: int, user_id: int) -> Dict[str, Any]:
        logger.debug(
            f"Fetching current question for session {session_id}, user {user_id}"
        )
        ...
        try:
            ...
            logger.error(
                f"Index mismatch in session {session_id}: current_index={session.current_index}, total={session.total_questions}"
            )
            raise HTTPException(status_code=500, detail="Internal session state error")

        # Find correct option id from MCQ options to avoid relying on link field
        correct_option = next(
            (opt for opt in mcq.options if getattr(opt, "is_correct", False)), None
        )
        if not correct_option:
            logger.error(f"MCQ {mcq.id} has no correct option defined")
            raise HTTPException(
                status_code=500, detail="Data integrity error: No correct option found"
            )
    ...
    # -------------------------
    def submit_answer(
        self, session_id: int, user_id: int, question_id: int, selected_option_id: int
    ) -> Dict[str, Any]:
        logger.info(
            f"Submitting answer for session {session_id}, question {question_id}, option {selected_option_id}"
        )
        session = self.session_repo.get_active(session_id, user_id)
        ...
        try:
            ...
            logger.error(
                f"Index mismatch during submission in session {session_id}: current_index={session.current_index}"
            )
            raise HTTPException(status_code=500, detail="Internal session state error")

        if mcq.id != question_id:
            logger.warning(
                f"Question mismatch in session {session_id}. Expected {mcq.id}, got {question_id}"
            )
        ...
        if not selected_option:
            logger.warning(
                f"Invalid option {selected_option_id} submitted for question {question_id}"
            )
    ...
    # -------------------------
    # 5️⃣ Get all questions for session
    # -------------------------
    def get_all_questions(self, session_id: int, user_id: int) -> List[Dict[str, Any]]:
        logger.debug(f"Fetching all questions for session {session_id}, user {user_id}")
        session = self.session_repo.get_active(session_id, user_id)

        # Build list sorted by order_index to preserve session order
        try:
            sorted_links = sorted(
                session.questions, key=lambda q: getattr(q, "order_index", 0)
            )
        except Exception:
            sorted_links = session.questions

        questions = []
        for link in sorted_links:
            mcq = link.mcq

            # Find correct option id from MCQ options
            correct_option = next(
                (opt for opt in mcq.options if getattr(opt, "is_correct", False)), None
            )
            if not correct_option:
                ...
                raise HTTPException(
                    status_code=500,
                    detail="Data integrity error: No correct option found",
                )
            ...
            questions.append(
                {
                    ...
                    "question_text": mcq.question_text,
                    "options": [
                        {"id": opt.id, "text": opt.option_text} for opt in mcq.options
                    ],
                    "correct_option_id": correct_option.id,
                    "explanation": mcq.explanation,
